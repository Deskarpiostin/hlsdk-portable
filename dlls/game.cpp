...
static bool g_coopProfileLoaded = false;
static int g_coopHudColor[3] = {0, 0, 0};
static char g_coopModelSubdir[64] = {0};
...
static bool ParseRGBLine(const std::string& line, int& r, int& g, int& b)
{
	return sscanf(line.c_str(), "%d %d %d", &r, &g, &b) == 3;
}
...
static std::string NormalizeMapName(const std::string& rawName)
{
	...
	const size_t slashPos = name.find_last_of("/\\");
	if (slashPos != std::string::npos)
		name = name.substr(slashPos + 1);

	if (name.size() > 4 && name.compare(name.size() - 4, 4, ".bsp") == 0)
...
{
	g_coopProfileLoaded = false;
	g_coopModelSubdir[0] = 0;
	...
	for (const std::string& profileFile : profileFiles)
	{
		...
		char relPath[256];
		safe_snprintf(relPath, sizeof(relPath), "xc/games/%s", fileName);
		...
		{
			if (NormalizeMapName(lines[i]) == normalizedCurrentMap)
			{
				mapMatch = true;
				break;
			}
		...
		int r = 0, g = 0, b = 0;
		if (!ParseRGBLine(lines[0], r, g, b))
			continue;
		r = Q_max(0, Q_min(255, r));
		g = Q_max(0, Q_min(255, g));
		b = Q_max(0, Q_min(255, b));

		const std::string& modelSubdir = lines[1];
		...
		g_coopHudColor[1] = g;
		g_coopHudColor[2] = b;
		strncpy(g_coopModelSubdir, modelSubdir.c_str(), sizeof(g_coopModelSubdir) - 1);
		g_coopModelSubdir[sizeof(g_coopModelSubdir) - 1] = 0;
		g_coopProfileLoaded = true;
		ALERT(at_console, "Loaded xc game profile %s for map %s\n", fileName, currentMap);
		break;
	}
...
{
	if (!g_coopProfileLoaded || !modelPath || !*modelPath)
		return modelPath;

	if (strnicmp(modelPath, "models/", 7) != 0)
		return modelPath;

	if (strnicmp(modelPath + 7, g_coopModelSubdir, strlen(g_coopModelSubdir)) == 0 && modelPath[7 + strlen(g_coopModelSubdir)] == '/')
		return modelPath;

	static char remapped[256];
	safe_snprintf(remapped, sizeof(remapped), "models/%s/%s", g_coopModelSubdir, modelPath + 7);
	return remapped;
}

bool GetCoopHudColor(int& r, int& g, int& b)
{
	if (!g_coopProfileLoaded)
		return false;
	r = g_coopHudColor[0];
	g = g_coopHudColor[1];
	b = g_coopHudColor[2];
...
cvar_t mp_coop		= { "mp_coop","0", FCVAR_SERVER }; // enable coop mode on next map change/restart
...
extern void CoopRequestEraseMemory();

void Cmd_CoopEraseMemory()
{
	CoopRequestEraseMemory();
	ALERT(at_console, "Coop memory will be erased on next map change\n");
}
...
void GameDLLInit()
{
	...
	g_engfuncs.pfnAddServerCommand("coop_erase_memory", Cmd_CoopEraseMemory);
}
