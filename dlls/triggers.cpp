...
public:
	...
	void ChangeLevelNow( CBaseEntity *pActivator, CBaseEntity *pCaller = nullptr );
...
void CChangeLevel::Spawn()
{
	...
	if (g_pGameRules->IsCoOp() && mp_coop.value != 0)
	{
...
{
	if (!( pev->spawnflags & SF_CHANGELEVEL_USEONLY ) && g_pGameRules->IsCoOp() && mp_coop.value == 0)
	{
		if (*st_szPrevMap)
		{
...
{
	ChangeLevelNow( pActivator, pCaller );
}

void CChangeLevel::ChangeLevelNow( CBaseEntity *pActivator, CBaseEntity *pCaller )
{
	edict_t	*pentLandmark;
	//LEVELLIST levels[16];
	...
	CoopSetTransitionLandmark(st_szNextSpot);
